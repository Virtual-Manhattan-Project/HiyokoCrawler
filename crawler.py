...
headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_13_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.80 Safari/537.36',
...
def pull_video_id(streamer_id, month_range):  # 从新到旧,爬取从from_month月到to_month月的视频ID
    ...
    for month in range(month_range[0], month_range[1] + 1):
        r = requests.post(list_url, headers=headers, data=json.dumps(
...
def pull_video_data(video_id):  # 获得ID为video_id的视频的数据
    l = requests.post(data_url, headers=headers,
...
def pull_chat_data(video_id):  # 获得ID为video_id的聊天的数据
    l = requests.post(chatdata_url, headers=headers,
                      data=json.dumps({"video_id": video_id}))
